...
class CloudKitService {
    ...
    public func saveEvent(_ event: Events) async throws {
    ...
    public func fetchEvents() async throws -> [Events] {
        ...
        let events: [Events] = records.compactMap { record in
    ...
    public func updateEvent(_ event: Events) async throws {
    ...
    public func deleteEvent(_ event: Events) async throws {
        // Try to get the existing Event as a CKRecord.
        guard let fetchedRecord = try? await database.record(for: .init(recordName: event.id)) else {
